...
public static class SimplePool
    {
        ...
        class Pool
        {
            ...
            public GameObject Spawn(Transform pos)
            {
                ...
                if (inactive.Count == 0)
                {
                    ...
                    obj = (GameObject)GameObject.Instantiate(prefab, pos);
                    obj.name = prefab.name + " (" + (nextId++) + ")";

                    // Add a PoolMember component so we know what pool
                    // we belong to.
                    obj.AddComponent<PoolMember>().myPool = this;
                    Debug.Log("Current PoolMember for " + obj.name + " " + obj.GetComponent<PoolMember>());
                }
                else
                {
            ...
            {
                Debug.Log("Current PoolMember when Despawn for" + obj.name + " " + obj.GetComponent<PoolMember>());
                obj.SetActive(false);
        ...
        /// call that function.
        /// </summary>
        static public GameObject Spawn(GameObject prefab, Transform pos)
        {
            Init(prefab);
